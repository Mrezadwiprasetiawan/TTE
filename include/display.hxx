...
class Display {
  ...
  int content_width() const { return lineNumbering ? width - lnWidth : width; }
...
public:
  ...
  void move_cursor(int r, int c) { cursorPos = {r, c}; }
  ...
  std::array<int, 2> get_cursor_pos() const { return cursorPos; }

  void set_line_numbering(bool val) {
    lineNumbering = val;
    mark_changed();
  }
  bool get_line_numbering_state() const { return lineNumbering; }

  void set_ln_width(int w) {
    lnWidth = w;
    mark_changed();
  }
  int get_ln_width() const { return lnWidth; }
  ...
  void mark_changed() { isChanged = true; }

...
InputHandler::InputHandler() {
  ...
  if (hIn != INVALID_HANDLE_VALUE) GetConsoleMode(hIn, &inModeOrig);
#endif
  enable_raw();
  enable_mouse();
}
...
void InputHandler::dispatch(const Event &e) {
  switch (e.type) {
    case Event::Type::Resize:
      if (cbs.onResize) cbs.onResize(cbs.ctx, e.w, e.h);
      break;
    case Event::Type::Key:
      if (cbs.onKey) cbs.onKey(cbs.ctx, e);
      break;
    case Event::Type::MousePress:
      if (cbs.onMousePress) cbs.onMousePress(cbs.ctx, e);
      break;
    case Event::Type::MouseRelease:
      if (cbs.onMouseRelease) cbs.onMouseRelease(cbs.ctx, e);
      break;
    case Event::Type::MouseMove:
      if (cbs.onMouseMove) cbs.onMouseMove(cbs.ctx, e);
      break;
    case Event::Type::MouseScroll:
      if (cbs.onMouseScroll) cbs.onMouseScroll(cbs.ctx, e);
      break;
    default:
      if (cbs.onUnhandled) cbs.onUnhandled(cbs.ctx, e);
      break;
  }
}
...
void InputHandler::set_callbacks(InputCallbacks cbs) { this->cbs = std::move(cbs); }

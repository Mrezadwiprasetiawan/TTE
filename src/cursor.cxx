...
void Cursor::clamp_col_to_row() {
  Display &d      = Display::getInstance();
  int      len    = cur_row_len();
  int      maxCol = (len > d.startColData) ? std::min(d.content_width() + 1, len - d.startColData + 1) : 1;
...
void Cursor::move_relative(Dir dir, int dist) {
  ...
  switch (dir) {
    ...
    case Dir::BOT: {
      ...
      if (target >= (int)d.data.size()) break;
...
void Cursor::go_line_start() {
  ...
  d.startColData    = 0;
  pos[1]            = 1;
  if (scrolled) d.mark_changed();
}

void Cursor::go_line_end() {
  Display &d       = Display::getInstance();
  int      dataRow = get_data_pos()[0];
  if (dataRow >= (int)d.data.size()) return;
  ...
  if (d.startColData != prevColData) d.mark_changed();
}
